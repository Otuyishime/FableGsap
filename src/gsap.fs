...
type Gsap =
    ...
    static member inline config (configs: IConfig seq) = Interop.gsapApi.config (Interop.makeConfigVars configs)
    ...
    static member inline defaults (vars: IVar seq) = Interop.gsapApi.defaults (Interop.makeAnimationVars vars)
    ...
    static member inline delayedCall (seconds: int, callBack: unit -> unit) = Interop.gsapApi.delayedCall (seconds, callBack)
    ...
    static member inline from (target: ITarget, vars: IVars) = Interop.gsapApi.from (target, vars)
    ...
    static member inline fromTo (target: ITarget, fromVars: IVars, toVars: IVars) = Interop.gsapApi.fromTo (target, fromVars, toVars)
    
    /// Returns a tween associated with the given ID. Returns `None` if no tween or timeline has that ID.
    static member inline getTweenById (id: string) = id |>Interop.gsapApi.getById |> Helpers.TryGetTween
    
    /// Returns a timeline associated with the given ID. Returns `None` if no tween or timeline has that ID.
    static member inline getTimelineById (id: string) = id |>Interop.gsapApi.getById |> Helpers.TryGetTimeline
    ...
    static member inline getTweensOf (target: ITarget) = Interop.gsapApi.getTweensOf target
    ...
    static member inline isTweening(tartget: ITarget) = Interop.gsapApi.isTweening tartget
    ...
    static member inline to_ (target: ITarget, vars: IVars) = Interop.gsapApi.to_ (target, vars)
    ...
    static member inline timeline (vars: IVars) = Interop.gsapApi.timeline vars

    /// Parses an easing string and returns the corresponding parsed easing function.
    static member inline parseEase (easeStr: string) = (Interop.gsapApi.parseEase easeStr)

    /// Immediately sets properties of the target(s) accordingly - essentially a zero-duration `to_()` tween
    /// with a more intuitive name
    static member inline set (target: ITarget, vars: IVars) = Interop.gsapApi.set (target, vars)
    
    /// Manually update the root (global) timeline. `This is only intended for advanced users.`
    static member inline updateRoot (seconds: int) = Interop.gsapApi.updateRoot seconds

    /// The GSAP version that is currently being used (in String form)
    static member inline version = Interop.gsapApi.version
    
    /// Register custom effects
    static member inline registerEffect (effect: IEffectProp seq) =
        console.log (Interop.makeEffect effect)
...
module Gsap =
    ...
    type globalTimeline = 
        ...
        static member inline pause (atTime: float, suppressEvents: bool) = Interop.gsapApi.globalTimeline.pause (atTime, suppressEvents)
